...
class heater:
    ...
    def __init__(self, config, target_temperature=0.0, init_on = False, DUT_PSU1 = None, DUT_PSU2 = None):
        ...
        try:
            ...
            # check if HEATERBLOCK section exists in config file:
            if 'HEATERBLOCK_disabled' in config:
                if 'HEATERBLOCK' not in config:
                    return
            else:
                raise KeyError('HEATERBLOCK')
            ...
            if 'HEATERBLOCK_disabled' in config: 
                ...
                logger.warning(f'Could not configure heaterblock because {e} is missing in the configuration file.')
                return
            pass

        except Exception as e:
            logger.warning(f'Could not configure heaterblock: {e}')
            pass
        ...
        if self._PSU != None:
            if self._power_is_on:
                try:
                    ...
                    logger.warning(f'Could not set heater power: {e}')
    ...
    def get_temperature(self, do_read = True):
        ...
        if self._TSENS is None:
            ...
            if do_read:
                ...
                if unit != 'deg.C':
                    raise ValueError(f'T value has wrong unit ({unit}).')
    ...
    def get_target_temperature_string(self, do_read = True):
        try:
            ...
            T = f"{T_tgt:.2f} ± {T_tol:.2f}" # convert and format numeric value to string
        except:
            T = "NA"
        return T
    ...
    def turn_on(self):
        ...
        if self._PSU != None:
            try:
                ...
                logger.warning(f'Could not turn on the heater: {e}')
    ...
    def turn_off(self):
        ...
        if self._PSU != None:
            try:
                ...
                logger.warning(f'Could not turn off the heater: {e}')
    ...
    def wait_for_stable_T(self, DUT_PSU_allowed_turn_off=None, terminal_output=False):
        ...
        if not self.is_on():
            ...
            while not self.temperature_is_stable():
                ...
                msg = f"Waiting for heaterblock temperature (CV/SV): {self.get_temperature_string()}/{self.get_target_temperature_string()} °C)..."
                print (msg, end="\r")
                # time.sleep(0.5)
            ...
            if not is_first_line:
                print (' '*len(msg), end="\r") # clear previous line from terminal, removing the heatup period from logs/terminal entirely (realtime output msg, not log)
...
class heater_control_thread(Thread):
    ...
    ### def __init__(self, heaterblock, interval_seconds=1):
        ...
        try:
            ...
            logger.warning(f'Could not initialize heaterblock:{e}')
            pass
    ...
    def run(self):
        ...
        try:
            ...
            logger.warning(f'Heaterblock PID controller failed: {e}')
            ...
            try:
                ...
                logger.warning(f'Could not turn off heaterblock: {e}')
                pass
