...
XINYI_SPECS = {
       ...
       #"RD6012P_12A":	( 0.0, 60.0, 12.0,  720,  0.001,  0.001,  0.0, 0.0, 1.8 ) , # 6012P in high-current mode (0..12A at 1 mA resolution), confirmed working
        ...
        "XY-6020L V5.1":( 0.0, 60.0, 20.0, 1200,  0.010,  0.010,  0.0, 0.0, 1.0 )   # V5.1 = HW-VER 25858
}
...
class XINYI(object):
    ...
    def __init__(self, port, baud=57600, modbus_addr=1, debug=True):
        '''
        PSU(port)
        ...
        modbus_addr=1:  address for device on multi-tenant ModBus connection
        ...
        try:
            self._instrument = minimalmodbus.Instrument(port=port, slaveaddress=modbus_addr, debug=debug)
            self._instrument.serial.baudrate = baud
            self._instrument.serial.timeout = 1.0
            self._instrument.debug = debug
            time.sleep(0.2) # wait a bit unit the port is really ready
        except Exception as e:
            logger.error(f"{e}: Could not connect to XinYi (XY) power supply with address {modbus_addr} at {port}")
        ...
        try:
            ...
            self.OCP_max = self.OVP_max = None
            hwmdl = self._get_register(XINYI_REGISTERS['HW-VER'])
            
            if hwmdl == 25858:
                # XY-6015L
                self.MODEL = 'XY-6020L V5.1'
                self.OVP_max = 61.0
                self.OCP_max = 15.0
 
            else:
                # unknown XinYi model:
                logger.warning(f'Unknown XinYi model ID: {hwmdl}')
                self.MODEL = '<unknown>'
            ...
            logger.error(f"{eKey}: Unknown XiNYi powersupply type/model: H/W {XINYI_REGISTERS['HW-VER']}, S/W {XINYI_REGISTERS['SW-VER']}.")
            raise RuntimeError(eKey)
        except Exception as e:
            logger.error(f"{e}: Could not determine XinYi powersupply type/model")
            raise RuntimeError(e)
        ...
        MEM = XINYI_REGISTERS['MEMORY_PROFILES']

        if ( self.OCP_max is None ) or ( self.OVP_max is None ):
            logger.warning(f"PSU model {self.MODEL} does not have defined OVP/OCP limits. Cannot set OVP/OCP.")
        else:
            logger.info (f"Setting runtime OVP/OCP to {self.OVP_max} V/{self.OCP_max} A")
            logger.info (f"Using registers {MEM['Cd0']['S-OVP']} and {MEM['Cd0']['S-OCP']} for this.")
            
            mul_U = self._voltage_multiplier()
            mul_I = self._current_multiplier()
            
            self._set_register(MEM['Cd0']['S-OVP'], self.OVP_max*mul_U)
            self._set_register(MEM['Cd0']['S-OCP'], self.OCP_max*mul_I)

            readback = self._get_N_registers(MEM['Cd0']['S-OVP'], 2)

            try:
                logger.debug(f"Register read-back OVP={readback[0]}, OCP={readback[1]}")
                if readback[0] is not self.OVP_max/mul_U or readback[1] is not self.OCP_max/mul_I: 
                    raise Exception
            except Exception as e:
                logger.error(f"{e}: OVP/OCP registers did not retain commanded value.")
                pass
    ...
    def _set_register(self, register, value):
        ...
        if k > MAX_COMM_ATTEMPTS:
            raise RuntimeError(f"Communication with XinYi {self.MODEL} at {self._instrument.serial.port} failed.")
    ...
    def output(self, state):
        ...
        self._set_register(XINYI_REGISTERS['OUT_EN'], state)
    ...
    def voltage(self, voltage):
        ...
        self._set_register(XINYI_REGISTERS['V-SET'], round(voltage*self._voltage_multiplier()))
    ...
    def current(self, current):
        ...
        self._set_register(XINYI_REGISTERS['I-SET'], round(current*self._current_multiplier()))
    ...
    def reading(self):
        ...
        u = self._get_N_registers(XINYI_REGISTERS['V-OUT'],2) # V-OUT and I-OUT are directly consecutive in the register list
        V = u[0] / V_mult
        I = u[1] / I_mult
        
        # check register 17 (CV or CC?)
        if self._get_register(XINYI_REGISTERS['CV_CC']) == 1:
            S = 'CC'
        else:
            S = 'CV'
