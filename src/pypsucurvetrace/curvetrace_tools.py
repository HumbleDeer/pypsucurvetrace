...
def __convert_str_tuple(x):
    # convert string x = "(1,2,3)" to tuple of floats y = (1.0, 2.0, 3.0)
        output = tuple(float(s) for s in x.strip("()").split(","))
...
def connect_PSU(configTESTER, label, logger):
    ...
    if not (label in configTESTER):
        ...
        # read Modbus address for XinYi PSUs (optional):
        modbus_addr = None
        if 'MODBUS_ADDR' in configTESTER[label]:
            try:
                modbus_addr = int(configTESTER[label]['MODBUS_ADDR'])
            except:
                logger.warning(f'Invalid MODBUS_ADDR in {label} configuration.')
                pass
        ...
        if 'TYPE' in configTESTER[label]:
            ...
            logger.error('No or empty TYPE field in the PSU configuration file! Cannot continue.')
            exit()
        if num_PSU > 1:
            try:
                x = eval(commandset)
                ...
                logger.error("Could not parse TYPEs of stacked PSUs.")
                exit()
        ...
        # connect to PSU(s):
        logger.info (f'Connecting to power supply {label} ...')
        P = powersupply.PSU(port, commandset, label, V_SET_CALPOLY, V_READ_CALPOLY, I_SET_CALPOLY, I_READ_CALPOLY, modbus_addr)
        ...
        if 'NUMSTABLEREAD' in configTESTER[label]:
            ...
            logger.warning ('Number of consistent measurement readings (NUMSTABLEREADINGS) not configured! [default = 1]]')
            P.NSTABLEREADINGS = 1
        ...
        if P.CONNECTED:
            ...
            for k in range(len(P._PSU)):
                if len(P._PSU) > 1:
                    logger.info(f'* Command set (unit {str(k+1)}): {P._PSU[k].COMMANDSET}')
                    logger.info(f'* Model (unit {str(k+1)}): {P._PSU[k].MODEL}')
                else:
                    logger.info(f'* Command set: {P._PSU[k].COMMANDSET}')
                    logger.info(f'* Model: {P._PSU[k].MODEL}')
            logger.info(f'* Min/Max voltage SET: {P.VMIN} / {P.VMAX} V')
            logger.info(f'* Max. current: {P.IMAX} A')
            logger.info(f'* Max. power: {P.PMAX} W')
            logger.info(f'* Voltage SET/READ resolution: {P.VRESSET} / {P.VRESREAD} V')
            logger.info(f'* Current SET/READ resolution: {P.IRESSET} / {P.IRESREAD} A')
            logger.info(f'* Number of consistent readings for measurements: {P.NSTABLEREADINGS}')
